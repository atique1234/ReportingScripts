...
/****** Object:  StoredProcedure [dbo].[SALES_BY_BOOKING_CURRENCY_LIVE]    Script Date: 10/22/2015 12:33:09 ******/
SET ANSI_NULLS ON
GO
...
(CASE WHEN CARRIERCODE = 'D7' then CAP.CAPACITY*ISNULL(ISNULL(DIS.DISTANCE,REV_DIS.DISTANCE),DIS2.DISTANCE)
 ...
 END) ASK 
  ...
  (
	 select  
		isnull(carr_map.mappedcarrier ,IL.CARRIERCODE) CARRIERCODE,CONVERT(DATE,IL.DEPARTUREDATE,110) DEPARTUREDATE,
		 ...
		 (select IL.* 
		  ...
		  Group by isnull(carr_map.mappedcarrier ,IL.CARRIERCODE),CONVERT(DATE,IL.DEPARTUREDATE,110),
