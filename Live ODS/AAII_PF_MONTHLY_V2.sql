...
/****** Object:  StoredProcedure [dbo].[AAII_PF_MONTHLY_V2]    Script Date: 10/26/2015 10:34:47 ******/
SET ANSI_NULLS ON
GO
...
-- =================================================
-- SCRIPT BY: ABDULLAH AL MAHBUB
-- SCRIPT DATE: 13-03-2015
-- SCRIPT: PREMIUM FLEX MONTHLY SALES DATA
-- =================================================




--ALTER PROCEDURE [dbo].[AAII_PF_MONTHLY_V2]
--AS
	...
	SET @startDateUTC = CAST(CONVERT(VARCHAR, DATEADD(month, DATEDIFF(month, 0, @MYDateUTC)-1, 0),101) as datetime)
	SET @endDateUTC   = CAST(CONVERT(VARCHAR, DATEADD(month, DATEDIFF(month, 0, @MYDateUTC), 0),101) as datetime)
	print @MYDateUTC 
	
	--SET @startDateUTC = '2014-09-01'
	--SET @endDateUTC = '2014-10-01'
...
BP.PASSENGERID,
PJS.SEGMENTID,PJS.DEPARTUREDATE,PJS.DEPARTURESTATION,PJS.ARRIVALSTATION,PJS.JOURNEYNUMBER,PJS.SEGMENTNUMBER,PJS.INTERNATIONAL,PJS.FARECLASSOFSERVICE,PJS.ProductClassCode,
AR.ROLECODE,
IL.CARRIERCODE,
...
BEGIN TRY DROP TABLE TEMP_PF_MONTHLY_V2 END TRY BEGIN CATCH END CATCH

SELECT CONVERT(DATE, BOOKINGDATE,110) CAPTUREDDATE, DATEPART(YYYY,BOOKINGDATE) BOOKINGYEAR, DATENAME(MM,BOOKINGDATE) BOOKINGMONTH, DATENAME(WK,BOOKINGDATE) BOOKINGWEEK, CARRIERCODE,
...
(CASE WHEN ProductClassCode = 'HF' THEN 'ONLINE_PF'
	  WHEN ProductClassCode = 'FS' AND FARECLASSOFSERVICE NOT IN ('B','H') THEN 'CORP_PF' ELSE 'NON_PF' END) PREMFLEX,
...
SUM(FUEL_REV_POSCURRENCY) FUEL_REV_POSCURRENCY
 INTO TEMP_PF_MONTHLY_V2
 FROM #PASSENGERALLCURRENCY
 GROUP BY CONVERT(DATE, BOOKINGDATE,110), DATEPART(YYYY,BOOKINGDATE), DATENAME(MM,BOOKINGDATE), DATENAME(WK,BOOKINGDATE), CARRIERCODE,
(CASE WHEN ASCII(SUBSTRING(DEPARTURESTATION,1,1)) < ASCII(SUBSTRING(ARRIVALSTATION,1,1)) THEN DEPARTURESTATION + ARRIVALSTATION
	  WHEN ASCII(SUBSTRING(DEPARTURESTATION,1,1)) > ASCII(SUBSTRING(ARRIVALSTATION,1,1)) THEN ArrivalStation + DepartureStation
	  ELSE 
		CASE WHEN ASCII(SUBSTRING(DEPARTURESTATION,2,1)) < ASCII(SUBSTRING(ARRIVALSTATION,2,1)) THEN DEPARTURESTATION + ARRIVALSTATION
...
(CASE WHEN ASCII(SUBSTRING(DEPARTURECOUNTRYCODE,1,1)) < ASCII(SUBSTRING(ARRIVALCOUNTRYCODE,1,1)) THEN DEPARTURECOUNTRYCODE +'-'+ ARRIVALCOUNTRYCODE
	 ...
	 END),
(CASE WHEN ProductClassCode = 'HF' THEN 'ONLINE_PF'
	  WHEN ProductClassCode = 'FS' AND FARECLASSOFSERVICE NOT IN ('B','H') THEN 'CORP_PF' ELSE 'NON_PF' END),
