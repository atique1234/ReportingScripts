...
--ALTER PROCEDURE [wb].[SAT_usp_AA_Update_RR_AAX_YoY_RoutePerformance]
...
BEGIN
	...
	----* Seat count, BaseRev_RM *----

		----* Seat count, BaseRev_RM @ ExtractionDate,Current Year *----
		
	BEGIN TRY TRUNCATE TABLE Temp_AAX_Weekly_Route_BaseRevRM DROP TABLE Temp_AAX_Weekly_Route_BaseRevRM END TRY BEGIN CATCH END CATCH 
	...
	--select top 100* from #Temp_AAX_Weekly_Route_BaseRevRM

		

		----* Seat count, BaseRev_RM @ ExtractionDate, Last Year *----
				
	BEGIN TRY TRUNCATE TABLE Temp_AAX_Weekly_Route_Y1_BaseRevRM DROP TABLE Temp_AAX_Weekly_Route_Y1_BaseRevRM END TRY BEGIN CATCH END CATCH 
	select CAST(Convert(VARCHAR,@CapturedDate, 112) as datetime) as CapturedDate,Year, Month, WeekNumber, 
	DATEPART(DW,@CapturedDate-1) as dayOfWeek, Cabin,
	A.PassengerID, RecordLocator, DepartureDate, CarrierCode, FlightNumber, Route,
	sum((ISNULL(Fare_Amt,0) + ISNULL(Disc_Amt,0) + ISNULL(Promo_Amt,0))*ISNULL(ConversionRate,1)) as BaseFare_RM,
	...
	--select top 10000* from #Temp_AAX_Weekly_Route_Y1_BaseRevRM
	--where CarrierCode = 'AK'
	--and FlightNumber = '5123' 
	--and StartWeek = '2010-01-10 00:00:00.000'
			
		----* Seat count, BaseRev_RM @ MonthEnd, Last Year *----
		
	BEGIN TRY TRUNCATE TABLE Temp_AAX_Weekly_Route_Y1M_BaseRevRM DROP TABLE Temp_AAX_Weekly_Route_Y1M_BaseRevRM END TRY BEGIN CATCH END CATCH 
	select CAST(Convert(VARCHAR,@CapturedDate, 112) as datetime) as CapturedDate,Year, Month, WeekNumber, 
	DATEPART(DW,@CapturedDate-1) as dayOfWeek, Cabin,
	A.PassengerID, RecordLocator, DepartureDate, CarrierCode, FlightNumber, Route,
	sum((ISNULL(Fare_Amt,0) + ISNULL(Disc_Amt,0) + ISNULL(Promo_Amt,0))*ISNULL(ConversionRate,1)) as BaseFare_RM,
	...
	--select top 1000* from #Temp_AAX_Weekly_Route_Y1M_BaseRevRM order by startWeek
	...
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_Ancillaries DROP TABLE #Temp_AAX_Weekly_Route_Ancillaries END TRY BEGIN CATCH END CATCH
	...
	Into #Temp_AAX_Weekly_Route_Ancillaries
	From Temp_AAX_Weekly_Route_BaseRevRM A 
	...
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_AncillariesRM DROP TABLE #Temp_AAX_Weekly_Route_AncillariesRM END TRY BEGIN CATCH END CATCH
	...
	Into #Temp_AAX_Weekly_Route_AncillariesRM
	from
		...
		from #Temp_AAX_Weekly_Route_Ancillaries
		where CarrierCode <> ' ' 
		and Flightnumber <> ' ' 
	...
	group by A.PassengerId,A.DepartureDate,A.CarrierCode,A.FlightNumber,A.DepartureStation,A.ArrivalStation, A.CurrencyCode
			...
			--select top 100*
			--from #Temp_AAX_Weekly_Route_AncillariesRM
	...
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_ALLRM DROP TABLE #Temp_AAX_Weekly_Route_ALLRM  END TRY BEGIN CATCH END CATCH
	...
	count(A.PassengerID)as SeatsSold, sum(BaseFare_RM) as BaseFare_RM, sum(FuelSurcharge_RM) as FuelSurcharge_RM, 
	--newly added
	...
	left join
		...
		from #Temp_AAX_Weekly_Route_AncillariesRM) B
	...
	A.CarrierCode, A.FlightNumber, A.Route, Cabin
		...
		--select top 10000* from #Temp_AAX_Weekly_Route_ALLRM 


		
		----* All Transactions @ ExtractionDate, Last Year *----
			
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_Y1_ALLRM DROP TABLE #Temp_AAX_Weekly_Route_Y1_ALLRM  END TRY BEGIN CATCH END CATCH
	...
	--newly added
	ISNULL(SUM(AncillaryAmount_RM),0) as AncillaryAmount_RM
	into #Temp_AAX_Weekly_Route_Y1_ALLRM 
	from
		(select distinct CapturedDate, Year, Month, WeekNumber, RecordLocator, 
		...
		ThruFare_RM, XHISEA_RM, ADM_RM
		--newly added
		from Temp_AAX_Weekly_Route_Y1_BaseRevRM) A
	left join
		(select distinct PassengerID, DepartureDate, CarrierCode, FlightNumber, route,
		AncillaryAmount_MYR as AncillaryAmount_RM
		from #Temp_AAX_Weekly_Route_AncillariesRM) B
	...
	--select top 10000* from #Temp_AAX_Weekly_Route_Y1_ALLRM 
	
	

		----* All Transactions @ MonthEnd, Last Year *----
	
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_Y1M_ALLRM DROP TABLE  #Temp_AAX_Weekly_Route_Y1M_ALLRM  END TRY BEGIN CATCH END CATCH
	select distinct CapturedDate, Year, Month, WeekNumber, A.DepartureDate, --RecordLocator, 
	A.CarrierCode, A.FlightNumber, A.Route, Cabin,
	count(A.PassengerID)as SeatsSold, sum(BaseFare_RM) as BaseFare_RM, sum(FuelSurcharge_RM) as FuelSurcharge_RM, 
	--newly added
	...
	--newly added
	ISNULL(SUM(AncillaryAmount_RM),0) as AncillaryAmount_RM
	into  #Temp_AAX_Weekly_Route_Y1M_ALLRM
	from
		(select distinct CapturedDate, Year, Month, WeekNumber, DepartureDate, RecordLocator, 
		CarrierCode, FlightNumber, Route, Cabin,
		PassengerID, BaseFare_RM, FuelSurcharge_RM,
		...
		ThruFare_RM, XHISEA_RM, ADM_RM
		--newly added
		from Temp_AAX_Weekly_Route_Y1M_BaseRevRM) A
	left join
		(select distinct PassengerID, DepartureDate, CarrierCode, FlightNumber, route,
		AncillaryAmount_MYR as AncillaryAmount_RM
		from #Temp_AAX_Weekly_Route_AncillariesRM) B
	...
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_ALL DROP TABLE  #Temp_AAX_Weekly_Route_ALL  END TRY BEGIN CATCH END CATCH
	...
	SUM(ISNULL(SeatsSold,0)) as SeatsSold, SUM(Capacity) as Capacity, SUM(ISNULL(BaseFare_RM,0)) as BaseFare_RM,
	SUM(ISNULL(FuelSurcharge_RM,0)) as FuelSurcharge_RM, 
	--newly added
	SUM(ISNULL(ThruFare_RM,0)) as ThruFare_RM, SUM(ISNULL(XHISEA_RM,0)) as XHISEA_RM, SUM(ISNULL(ADM_RM,0)) as ADM_RM,
	--newly added
	SUM(ISNULL(AncillaryAmount_RM,0)) as AncillaryAmount_RM
	into #Temp_AAX_Weekly_Route_ALL
	from
	...
	left join	
		...
		Cabin, SUM(BaseFare_RM) as BaseFare_RM, SUM(FuelSurcharge_RM) as FuelSurcharge_RM, 
		--newly added
		SUM(ThruFare_RM) as ThruFare_RM, SUM(XHISEA_RM) as XHISEA_RM, SUM(ADM_RM) as ADM_RM,
		--newly added
		SUM(SeatsSold) as SeatsSold, SUM(AncillaryAmount_RM) as AncillaryAmount_RM
		from #Temp_AAX_Weekly_Route_ALLRM 
	...
	group by A.YEAR, A.MONTH, A.WeekNumber, A.CarrierCode, A.Route, A.Cabin
		...
		--select * from #Temp_AAX_Weekly_Route_ALL
		--where Route = 'CTUKUL'
		--and Month = 'March'
		--order by WeekNumber
	...
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_Y1_ALL DROP TABLE  #Temp_AAX_Weekly_Route_Y1_ALL  END TRY BEGIN CATCH END CATCH
	select A.YEAR, A.MONTH, A.WeekNumber, A.CarrierCode,A.Route, A.Cabin, 
	SUM(ISNULL(SeatsSold_Y1,0)) as SeatsSold_Y1, SUM(Capacity_Y1) as Capacity_Y1, SUM(ISNULL(BaseFare_Y1_RM,0)) as BaseFare_Y1_RM,
	SUM(ISNULL(FuelSurcharge_Y1_RM,0)) as FuelSurcharge_Y1_RM, 
	--newly added
	SUM(ISNULL(ThruFare_Y1_RM,0)) as ThruFare_Y1_RM, SUM(ISNULL(XHISEA_Y1_RM,0)) as XHISEA_Y1_RM, SUM(ISNULL(ADM_Y1_RM,0)) as ADM_Y1_RM,
	--newly added
	SUM(ISNULL(AncillaryAmount_Y1_RM,0)) as AncillaryAmount_Y1_RM
	into #Temp_AAX_Weekly_Route_Y1_ALL
	from
		(select CarrierCode, FlightNumber, DepartureStation+ArrivalStation as Route,
		DepartureDate, Year(DepartureDate) as Year, DATENAME(MM, DepartureDate) as Month, DATENAME(WW,DepartureDate) as WeekNumber, 
	...
	left join
		(select YEAR, MONTH, WeekNumber, DepartureDate, CarrierCode, FlightNumber, Route, 
		Cabin, SUM(ISNULL(BaseFare_RM,0)) as BaseFare_Y1_RM, SUM(ISNULL(FuelSurcharge_RM,0)) as FuelSurcharge_Y1_RM,
		--newly added
		SUM(ISNULL(ThruFare_RM,0)) as ThruFare_Y1_RM, SUM(ISNULL(XHISEA_RM,0)) as XHISEA_Y1_RM, SUM(ISNULL(ADM_RM,0)) as ADM_Y1_RM,
		--newly added
		SUM(SeatsSold) as SeatsSold_Y1, SUM(ISNULL(AncillaryAmount_RM,0)) as AncillaryAmount_Y1_RM
		from  #Temp_AAX_Weekly_Route_Y1_ALLRM
		group by CapturedDate, Year, MONTH, WeekNumber, DepartureDate, CarrierCode, FlightNumber, Route, 
		Cabin) B
		on A.CarrierCode = B.CarrierCode and A.FlightNumber = B.FlightNumber 
	...
	group by A.YEAR, A.MONTH, A.WeekNumber, A.CarrierCode,A.Route, A.Cabin
		...
		--from #Temp_AAX_Weekly_Route_Y1_ALL
	...
	BEGIN TRY TRUNCATE TABLE #Temp_AAX_Weekly_Route_Y1M_ALL DROP TABLE  #Temp_AAX_Weekly_Route_Y1M_ALL  END TRY BEGIN CATCH END CATCH
	select A.YEAR, A.MONTH, A.WeekNumber, A.CarrierCode, A.Route, A.Cabin,
	SUM(ISNULL(SeatsSold_Y1M,0)) as SeatsSold_Y1M, SUM(Capacity_Y1M) as Capacity_Y1M, SUM(BaseFare_Y1M_RM) as BaseFare_Y1M_RM, 
	SUM(FuelSurcharge_Y1M_RM) as FuelSurcharge_Y1M_RM, 
	--newly added
	SUM(ISNULL(ThruFare_Y1M_RM,0)) as ThruFare_Y1M_RM, SUM(ISNULL(XHISEA_Y1M_RM,0)) as XHISEA_Y1M_RM, SUM(ISNULL(ADM_Y1M_RM,0)) as ADM_Y1M_RM,
	--newly added
	SUM(AncillaryAmount_Y1M_RM) as AncillaryAmount_Y1M_RM
	into #Temp_AAX_Weekly_Route_Y1M_ALL
	from
		(select CarrierCode, FlightNumber, DepartureStation+ArrivalStation as Route,
		DepartureDate, Year(DepartureDate) as Year, DATENAME(MM, DepartureDate) as Month, DATENAME(WW,DepartureDate) as WeekNumber, 
	...
	left join
		(select YEAR, MONTH, WeekNumber, DepartureDate, CarrierCode, FlightNumber, Route, 
		Cabin, SUM(BaseFare_RM) as BaseFare_Y1M_RM, SUM(FuelSurcharge_RM) as FuelSurcharge_Y1M_RM, 
		--newly added
		SUM(ISNULL(ThruFare_RM,0)) as ThruFare_Y1M_RM, SUM(ISNULL(XHISEA_RM,0)) as XHISEA_Y1M_RM, SUM(ISNULL(ADM_RM,0)) as ADM_Y1M_RM,
		--newly added
		SUM(SeatsSold) as SeatsSold_Y1M, SUM(AncillaryAmount_RM) as AncillaryAmount_Y1M_RM
		from #Temp_AAX_Weekly_Route_Y1M_ALLRM 
		group by CapturedDate, Year, MONTH, WeekNumber, DepartureDate, CarrierCode, FlightNumber, Route, 
		Cabin) B
		on A.CarrierCode = B.CarrierCode and A.FlightNumber = B.FlightNumber 
		and A.Route = B.Route and A.DepartureDate = B.DepartureDate
	...
	group by A.YEAR, A.MONTH, A.WeekNumber, A.CarrierCode, A.Route, A.Cabin
		...
		--from #Temp_AAX_Weekly_Route_Y1M_ALL
		--where Route in ('BDOKUL')
		--and Year = 2011
		...
		--select * from #Temp_AAX_Weekly_Route_ALL where Capacity is not null and Route = 'AORKUL' order by WeekNumber
		--select * into Temp_AAX_Weekly_Route_ALL from  #Temp_AAX_Weekly_Route_ALL  where Route = 'BKIKUL' and FlightNumber = '5123' and StartWeek = '2011-01-10 00:00:00.000'
		--select * into Temp_AAX_Weekly_Route_Y1_ALL from #Temp_AAX_Weekly_Route_Y1_ALL  where Route = 'BKIKUL' and FlightNumber = '5123' and StartWeek = '2010-01-10 00:00:00.000'
		--select * into Temp_AAX_Weekly_Route_Y1M_ALL from #Temp_AAX_Weekly_Route_Y1M_ALL where Route = 'BKIKUL' and FlightNumber = '5123' and StartWeek = '2010-01-10 00:00:00.000'
	...
	BEGIN TRY TRUNCATE TABLE SAT_Temp_AAX_Weekly_Route_final DROP TABLE SAT_Temp_AAX_Weekly_Route_final  END TRY BEGIN CATCH END CATCH
	...
	SeatsSold, Capacity, BaseFare_RM, FuelSurcharge_RM, 
	--newly added
	ThruFare_RM, XHISEA_RM, ADM_RM,
	--newly added
	AncillaryAmount_RM, 
	SeatsSold_Y1,Capacity_Y1, BaseFare_Y1_RM, FuelSurcharge_Y1_RM, 
	--newly added
	ThruFare_Y1_RM, XHISEA_Y1_RM, ADM_Y1_RM,
	--newly added
	AncillaryAmount_Y1_RM,
	SeatsSold_Y1M, Capacity_Y1M, BaseFare_Y1M_RM, FuelSurcharge_Y1M_RM, 
	--newly added
	ThruFare_Y1M_RM, XHISEA_Y1M_RM, ADM_Y1M_RM,
	--newly added
	AncillaryAmount_Y1M_RM
	into SAT_Temp_AAX_Weekly_Route_final
	from
		(select CapturedDate, A.YEAR, A.MONTH, A.WeekNumber, A.CarrierCode, A.Route2, 
		A.Cabin,  
		SeatsSold, Capacity, BaseFare_RM, FuelSurcharge_RM, /*newly added*/ThruFare_RM, XHISEA_RM, ADM_RM,/*newly added*/ AncillaryAmount_RM, 
		SeatsSold_Y1,Capacity_Y1, BaseFare_Y1_RM, FuelSurcharge_Y1_RM,/*newly added*/ThruFare_Y1_RM, XHISEA_Y1_RM, ADM_Y1_RM, /*newly added*/AncillaryAmount_Y1_RM,
		SeatsSold_Y1M, Capacity_Y1M, BaseFare_Y1M_RM, FuelSurcharge_Y1M_RM, /*newly added*/ThruFare_Y1M_RM, XHISEA_Y1M_RM, ADM_Y1M_RM, /*newly added*/AncillaryAmount_Y1M_RM
		from
			...
			ISNULL(BaseFare_RM,0) as BaseFare_RM, ISNULL(FuelSurcharge_RM,0) as FuelSurcharge_RM,
			--newly added
			ISNULL(ThruFare_RM,0) as ThruFare_RM, ISNULL(XHISEA_RM,0) as XHISEA_RM, ISNULL(ADM_RM,0) as ADM_RM,
			--newly added
			ISNULL(AncillaryAmount_RM,0) as AncillaryAmount_RM
			from #Temp_AAX_Weekly_Route_ALL
		...
		left join
			...
			ISNULL(BaseFare_Y1_RM,0) as BaseFare_Y1_RM, 
			ISNULL(FuelSurcharge_Y1_RM,0) as FuelSurcharge_Y1_RM,
			--newly added
			ISNULL(ThruFare_Y1_RM,0) as ThruFare_Y1_RM, ISNULL(XHISEA_Y1_RM,0) as XHISEA_Y1_RM, ISNULL(ADM_Y1_RM,0) as ADM_Y1_RM,
			--newly added
			ISNULL(AncillaryAmount_Y1_RM,0) as AncillaryAmount_Y1_RM
			from #Temp_AAX_Weekly_Route_Y1_ALL
		...
		left join
			...
			ISNULL(BaseFare_Y1M_RM,0) as BaseFare_Y1M_RM, 
			ISNULL(FuelSurcharge_Y1M_RM,0) as FuelSurcharge_Y1M_RM,
			--newly added
			ISNULL(ThruFare_Y1M_RM,0) as ThruFare_Y1M_RM, ISNULL(XHISEA_Y1M_RM,0) as XHISEA_Y1M_RM, ISNULL(ADM_Y1M_RM,0) as ADM_Y1M_RM,
			--newly added
			ISNULL(AncillaryAmount_Y1M_RM,0) as AncillaryAmount_Y1M_RM
			from #Temp_AAX_Weekly_Route_Y1M_ALL
		...
		A.Cabin,
		/*  
		SeatsSold, Capacity, BaseFare_RM, FuelSurcharge_RM, AncillaryAmount_RM, 
		SeatsSold_Y1,Capacity_Y1, BaseFare_Y1_RM, FuelSurcharge_Y1_RM, AncillaryAmount_Y1_RM,
		SeatsSold_Y1M, Capacity_Y1M, BaseFare_Y1M_RM, FuelSurcharge_Y1M_RM, AncillaryAmount_Y1M_RM
		*/
		SeatsSold, Capacity, BaseFare_RM, FuelSurcharge_RM, /*newly added*/ThruFare_RM, XHISEA_RM, ADM_RM,/*newly added*/ AncillaryAmount_RM, 
		SeatsSold_Y1,Capacity_Y1, BaseFare_Y1_RM, FuelSurcharge_Y1_RM,/*newly added*/ThruFare_Y1_RM, XHISEA_Y1_RM, ADM_Y1_RM, /*newly added*/AncillaryAmount_Y1_RM,
		SeatsSold_Y1M, Capacity_Y1M, BaseFare_Y1M_RM, FuelSurcharge_Y1M_RM, /*newly added*/ThruFare_Y1M_RM, XHISEA_Y1M_RM, ADM_Y1M_RM, /*newly added*/AncillaryAmount_Y1M_RM
		...
		left join
			...
			ISNULL(BaseFare_RM,0) as BaseFare_Y1M_RM, 
			ISNULL(FuelSurcharge_RM,0) as FuelSurcharge_Y1M_RM,
			--newly added
			ISNULL(ThruFare_RM,0) as ThruFare_Y1M_RM, ISNULL(XHISEA_RM,0) as XHISEA_Y1M_RM, ISNULL(ADM_RM,0) as ADM_Y1M_RM,
			--newly added
			ISNULL(AncillaryAmount_RM,0) as AncillaryAmount_Y1M_RM
			from #Temp_AAX_Weekly_Route_ALL
	...
	left join
		...
		from REZAKWB01.WB.SAT_vw_TravelCountry) F
		on H.CarrierCode = F.CarrierCode and H.Route2 = F.Route
	left join
		(select distinct Country, Region
		from REZAKWB01.WB.SAT_vw_Region) G
		on LTRIM(RTRIM(F.Country)) = LTRIM(RTRIM(G.Country))
		
		update SAT_Temp_AAX_Weekly_Route_final
		set Distance = Distance_km 
		from  AAX_Distance B
		where Route = ODPair
		
	
		--select * from SAT_Temp_AAX_Weekly_Route_final where Route = 'HGHKUL'
		--select * from #Temp_AAX_Weekly_Route_ALL where Capacity is not null and Route = 'AORKUL' order by WeekNumber
		--select * into Temp_AAX_Weekly_Route_ALL from  #Temp_AAX_Weekly_Route_ALL  where Route = 'BKIKUL' and FlightNumber = '5123' and StartWeek = '2011-01-10 00:00:00.000'
		--select * into Temp_AAX_Weekly_Route_Y1_ALL from #Temp_AAX_Weekly_Route_Y1_ALL  where Route = 'BKIKUL' and FlightNumber = '5123' and StartWeek = '2010-01-10 00:00:00.000'
		--select * into Temp_AAX_Weekly_Route_Y1M_ALL from #Temp_AAX_Weekly_Route_Y1M_ALL where Route = 'BKIKUL' and FlightNumber = '5123' and StartWeek = '2010-01-10 00:00:00.000'
