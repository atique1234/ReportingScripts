...
/****** Object:  StoredProcedure [wb].[AAII_Ancillary_Revenue_by_Flight_Date]    Script Date: 10/26/2015 10:32:49 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @endDateUTC			 datetime,
	...
	--SET @departureStart = '2014-05-01'
	--SET @departureEnd = '2015-01-01'
...
PJL.INVENTORYLEGID,
IL.CARRIERCODE,IL.FLIGHTNUMBER,PJS.INTERNATIONAL,DATEDIFF(MI,IL.STDUTC,IL.STAUTC) FLIGHT_DURATION
INTO #PASSENGERS
FROM
...
FL.CARRIERCODE,FL.FLIGHTNUMBER,FL.INTERNATIONAL,FL.FLIGHT_DURATION,FL.FEENUMBER,FL.FEECODE,FL.LOCALCURRENCYCODE,
F.NAME SSRDESC,
...
SELECT DEPARTUREDATE,CARRIERCODE,FLIGHTNUMBER,
...
FROM #FEEALL
GROUP BY DEPARTUREDATE,CARRIERCODE,FLIGHTNUMBER,
(CASE WHEN ASCII(SUBSTRING(DEPARTURESTATION,1,1)) < ASCII(SUBSTRING(ARRIVALSTATION,1,1)) THEN DEPARTURESTATION + ARRIVALSTATION
	  WHEN ASCII(SUBSTRING(DEPARTURESTATION,1,1)) > ASCII(SUBSTRING(ARRIVALSTATION,1,1)) THEN ArrivalStation + DepartureStation
	  ELSE 
		CASE WHEN ASCII(SUBSTRING(DEPARTURESTATION,2,1)) < ASCII(SUBSTRING(ARRIVALSTATION,2,1)) THEN DEPARTURESTATION + ARRIVALSTATION
...
SELECT AR.*,HUB.HUB,P.NO_OF_PAX,IL.CAPACITY 
...
ODS.INVENTORYLEG IL
ON
AR.DEPARTUREDATE = IL.DEPARTUREDATE
AND
AR.CARRIERCODE = IL.CARRIERCODE
AND
AR.SECTOR = IL.DEPARTURESTATION+IL.ARRIVALSTATION
AND
AR.FLIGHTNUMBER = IL.FLIGHTNUMBER
AND
AR.FLIGHT_DURATION = DATEDIFF(MI,IL.STDUTC,IL.STAUTC)
LEFT JOIN
